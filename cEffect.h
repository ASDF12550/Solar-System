...
class cEffect {
    ...
    float AngleX = 0, AngleY = 0, Speed = 0, LookAngleX = 0, LookAngleY = 0, Timer = 10000 - rand() % 1000;
public:
    float Size = .1;
    float x, y, z;
    bool Dead = 0;
    cEffect(unsigned int* Texture, float AngleX, float AngleY, float Speed, float x, float y, float z)
    ...
    void UpDate() { // UpDate
        ...
        if (Timer > 0) Timer -= .5; else Dead = 1;
